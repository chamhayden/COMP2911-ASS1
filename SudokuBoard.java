...
public class SudokuBoard implements Board {
	...
	public void generate(int difficulty)
	{
		...
		//TODO I WASN'T SURE WHAT THIS WAS DOING. 
		//IT LOOKS LIKE ITS TRYING TO GENERATE A RANDOM BOARD
		//ITS MESSING UP MY GENERATOR SO I COMMENTED IT OUT
		/*
		// Start Filler Shit
	...
	{
		//TODO I ADDED THESE TO ALL USER FUNCTIONS 
		//THIS MAY CAUSE AN ISSUE IF YOU CALL THEM INTERNALLY LATER
		//I.E. YOU HAVE ALREADY DECREMENTED
		row--;
		col--;
	...
	//TODO THIS WAS RETURNING VOID SO I MADE IT USEFULL
	...
	public boolean squareHas(int sqrId, int value)
	{
		...
		sqrId--;
		int xStart = sqrId%3*3;
		int yStart = (int)Math.floor(sqrId/3)*3;
		boolean result = false;
		int x, y;
		...
		while(y < yStart + 3){
			while(x < xStart + 3){
				if(getCellValue(x+1, y+1) == value){
					if (isInitiallyVisibleCell(x+1, y+1)){
						return true;
					}
				}
				x++;
		...
		for (int i = 0; i < this.boardSize; i++)
		{
			for (int j = 0; j < this.boardSize; j++)
			{
				...
				{
					if (getCellValue(i-1, j-1) == value)
					{
						System.out.println("<");
						if(isInitiallyVisibleCell(i-1, j-1)){
							System.out.println(">");
							result = true;
						}
					}
